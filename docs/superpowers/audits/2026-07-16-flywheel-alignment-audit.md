...
>
> **⚠️ Amendment 2026-07-16（基准补充,见 §九）**：客服代理（起草回复/自动发送/客户记忆）**同为客户需求**,并且是未来"视觉外挂客服"衍生场景的种子——本审计中所有"CS 栈=第二产品线/范围债"的措辞按 §九 修正为"平行场景"。缺口矩阵、基底判定、路线图顺序**不受影响**。
...
## 九、Amendment（2026-07-16）：平台论点确立,CS 栈翻案

**基准补充**：客户澄清——客服代理同为其需求（记录日常工作/学习习惯 + 为"视觉外挂客服"衍生功能做准备）；产品意图是**一套工具服务多场景多人群,功能模块接近**。

**Planner 判定：平台论点成立,但只在底盘层成立。**

1. **底盘层（成立）**：视觉采集/IPC/事件底座/脱敏机制/记忆基建为两场景共用——审计已发现"最硬的四分之一恰好共用",这就是平台论点的代码证据。
2. **数据策略层（不成立,必须参数化）**：两场景对基底反向拉扯——隐私主体（保护客户 PII vs 操作者自我追踪）、保留策略（审计尾迹可轮转 vs 永不删除）、记忆键控轴（按客户 vs 按商品/品类/会话）。不做按场景 profile 的策略参数化,场景间会互相污染数据保证（实证：P3 为 CS 加强的脱敏销毁了选品 join key,即本审计 S2）。**平台的正确形态 = 一套底盘 + per-profile 策略层。**
3. **CS 栈处置翻案**：§五"隔离（第二产品线,范围债）"改判为"**场景 profile 化**"——工程动作不变（build target/config flag 拆分）,意义变更：客服 profile 与选品 profile 均为长期出货的一等公民。
4. **衍生优先级注记**：视觉外挂客服离交付**远比选品近**——安全门/联系人复核/客户记忆/回复代理已经过多轮对抗审查加固,端到端可跑；选品场景则四器官全缺。衍生产品化时客服外挂可先行。
5. **路线图影响**：顺序不变（P7.1→7.3 仍是关键路径）；P7.5 更名为"场景 profile 化 + 量化输出"；MemPalace `(entity_type, entity_id)` 键控泛化权重上升（平台论点下是记忆层跨场景复用的开关）,仍不阻塞第一圈,P7.6 后立项。

## 十、Amendment 2（2026-07-16）：隐私投入冻结决策（Owner 拍板）

**决策**：功能优先,暂不投入隐私工程;数据保持本地;功能完成后由 Owner 自研的 `llm-privacy-filter`（tuomin,可逆脱敏代理）统一接管隐私层。

**Planner 判定：合理,采纳。** 与审计同向——S2（脱敏销毁 join key）因此直接消解,P7.1 砍掉"类型化占位符"设计。

**执行精确化（写给 Cursor,防止执行歪）**：
1. **冻结投入 ≠ 拆已建成的门。** 不许碰：自动发送三重安全门（防发错人=产品正确性）、白名单 fail-closed + 窗口作用域截屏（防截密码管理器=采集正确性）、CS 路径既有脱敏（已建成已测试,拆=负功）。LLM 出口的既有 Redactor 保持现状运行。
2. **"不考虑隐私"= 不新增隐私工作**：habit 事件明文本地落盘、URL/窗口标题原样存、不设计新遮蔽规则。
3. **三个 seam 必须保留**,使 P7.7 后接为配置级而非重写：LLM 单出口（tooluse.py/llm.py）、磁盘单出口（EventLog/SQLite）、按事件类别的策略开关（策略暂全 pass-through）。
4. **已知例外路径（有意接受）**：习惯数据 OCR 文本经 LLM API（当前 DeepSeek）出机,由既有 Redactor 兜底,P7.7 升级为可逆代理。

**路线图变更**：P7.1 的 S2 项简化为"habit 事件 pass-through + 保留策略开关"；新增 **P7.7 隐私回归**（tuomin 加 `/v1/messages` Anthropic 协议 → LLM 出口指向代理 → 可逆脱敏 + per-profile 策略；商业注记：llm-privacy-filter 成为平台可售隐私附加层,与按设备定价搭配）。其余阶段不变。
